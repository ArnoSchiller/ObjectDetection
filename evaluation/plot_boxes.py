...
def plot_boxes_on_image(image, boxes, classes=None, mode="xyrb", relative=False):
    """
    mode : 
...
imgs_path = os.path.join("..", "datasets", "dataset-v2-dih4cps", "version_1", "voc", "test")
model_name = "yolov5l_dsv2"
...
# pick random image 
image_files = glob.glob(imgs_path+"/*.png")
rand_idxs =  np.random.randint(0, len(image_files)-1, 10)
...
for idx, path in enumerate(img_paths):
    ...
    if annotation:
        plot_boxes_on_image(image=im, boxes=annotation[0],mode="xywh", relative=True)
        print(annotation[1], annotation[2])
